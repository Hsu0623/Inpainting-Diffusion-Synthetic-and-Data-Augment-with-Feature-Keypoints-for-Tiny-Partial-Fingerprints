...
def load_data_inpa(
    ...
    ** kwargs
    ...
    print(f'loading {len(gt_paths)} images.')
    classes = None
    if class_cond:
...
class ImageDatasetInpa(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        arr_gt = arr_gt.reshape((arr_gt.shape[0],arr_gt.shape[1],1))
        arr_mask = np.array(pil_mask)
        arr_mask = arr_mask.reshape((arr_mask.shape[0],arr_mask.shape[1],1))
